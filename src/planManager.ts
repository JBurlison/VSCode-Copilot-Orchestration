...
import { Plan, UserStory, Task, TaskStatus, Priority, PlanChangeEvent } from './types';
...
export class PlanManager {
    ...
    private loadPlan(): Plan {
        const storedPlan = this.context.workspaceState.get<Plan>('orchestration.plan');
        if (storedPlan) {
            return storedPlan;
        }
    ...
    public async updatePlan(updates: { name?: string; description?: string }): Promise<Plan> {
    ...
    public async createUserStory(input: {
        ...
        await this.savePlan();
        this._onPlanChanged.fire({ plan: this.plan, changeType: 'userStory', changedItem: userStory });
        return userStory;
    }

    public async updateUserStory(userStoryId: string, updates: Partial<Omit<UserStory, 'id' | 'createdAt'>>): Promise<UserStory | undefined> {
        const storyIndex = this.plan.userStories.findIndex(s => s.id === userStoryId);
        if (storyIndex === -1) {
            return undefined;
        }
        ...
        const updatedStory: UserStory = {
            ...
            updatedAt: now,
            completedAt: updates.status === 'completed' && story.status !== 'completed' ? now : story.completedAt
        };

        this.plan.userStories[storyIndex] = updatedStory;
        this.plan.updatedAt = now;
        await this.savePlan();
        this._onPlanChanged.fire({ plan: this.plan, changeType: 'userStory', changedItem: updatedStory });
        return updatedStory;
    }
    ...
    public async deleteUserStory(userStoryId: string): Promise<boolean> {
        ...
        this.plan.tasks = this.plan.tasks.filter(t => t.userStoryId !== userStoryId);
    ...
    public async createTask(input: {
        ...
        this.plan.updatedAt = now;
        await this.savePlan();
        this._onPlanChanged.fire({ plan: this.plan, changeType: 'task', changedItem: task });
        return task;
    }

    public async updateTask(taskId: string, updates: Partial<Omit<Task, 'id' | 'createdAt'>>): Promise<Task | undefined> {
        ...
        const updatedTask: Task = {
            ...
            updatedAt: now,
            completedAt: updates.status === 'completed' && task.status !== 'completed' ? now : task.completedAt
        };

        this.plan.tasks[taskIndex] = updatedTask;
        this.plan.updatedAt = now;
        await this.savePlan();
        this._onPlanChanged.fire({ plan: this.plan, changeType: 'task', changedItem: updatedTask });
        return updatedTask;
    }
    ...
    public async moveTask(taskId: string, newUserStoryId: string): Promise<Task | undefined> {
